    ...
    'url':'/maintain_monitor/maintain-monitor-info/',
     success:function(result){
            if(result['code'] === 200){
                ...
                limit_data_count = maintain_get_data.limit_value1[0]
                limit_data_date = maintain_get_data.limit_value2[0]
                ...
                for(var i=0; i<maintain_data.length; i++){
                    ...
                    if(maintain_data[i].NextCheckDate == null){maintain_data[i].NextCheckDate ="0T"}
                    ...
                    if(limit_data_count.length != 0 && limit_data_date.length != 0){
                        if(maintain_data[i].stand_count > limit_data_count.Max && maintain_data[i].stand_date > limit_data_date.Max ){
                            status = "正常"
                            status_class = "badge badge-success";}
                        if((0 < maintain_data[i].stand_count && maintain_data[i].stand_count <= limit_data_count.Max) ||
                           (0 < maintain_data[i].stand_date && maintain_data[i].stand_date<= limit_data_date.Max)){
                            status = "预警"
